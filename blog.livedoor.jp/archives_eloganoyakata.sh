...
while [ ! -z "$1" ]; do
    ...
    while [ 1 -eq 1 ]; do
        # http://blog.livedoor.jp/eloganoyakata/archives/26886600.html
        BEGINCOUNT=`${GREP} -- '<div class="entry-body">' ${FILENAME} | ${WC} -l | ${TR} -d ' '`
        ENDCOUNT=`${GREP} -- '-- entry_end --' ${FILENAME} | ${WC} -l | ${TR} -d ' '`
        if [ ${BEGINCOUNT} -gt 0 -a ${ENDCOUNT} -gt 0 ]; then
            ${CAT} ${FILENAME} | \
              ${SED} 's/></>\n</g' | \
              ${GREP} -m 1 -A 10000 -- '<div class="entry-body">' | \
              ${GREP} -m 1 -B 10000 -- '-- entry_end --' | \
