...
while [ ! -z "$1" ]; do
    ...
    while [ 1 -eq 1 ]; do
        # http://blog.livedoor.jp/avinfolie/archives/16573034.html
        BEGINCOUNT=`${GREP} -- '-- EntryImgSauce --' ${FILENAME} | ${WC} -l | ${TR} -d ' '`
        ENDCOUNT=`${GREP} -- '-- //EntryImgSauce --' ${FILENAME} | ${WC} -l | ${TR} -d ' '`
        if [ ${BEGINCOUNT} -gt 0 -a ${ENDCOUNT} -gt 0 ]; then
            ${CAT} ${FILENAME} | \
              ${SED} 's/></>\n</g' | \
              ${GREP} -m 1 -A 10000 -- '-- EntryImgSauce --' | \
              ${GREP} -m 1 -B 10000 -- '-- //EntryImgSauce --' | \
        ...
        # old http://blog.livedoor.jp/avinfolie/archives/27770110.html
        BEGINCOUNT=`${GREP} -- '<p class="article_title">' ${FILENAME} | ${WC} -l | ${TR} -d ' '`
        ENDCOUNT=`${GREP} -- '<div class="sidebottom">' ${FILENAME} | ${WC} -l | ${TR} -d ' '`
        if [ ${BEGINCOUNT} -gt 0 -a ${ENDCOUNT} -gt 0 ]; then
            ${CAT} ${FILENAME} | \
              ${SED} 's/></>\n</g' | \
              ${GREP} -m 1 -A 10000 -- '<p class="article_title">' | \
              ${GREP} -m 1 -B 10000 -- '<div class="sidebottom">' | \
              ${GREP} -i -e 'jpg' -e 'jpeg' -e 'png' -e 'gif' | \
                ${GREP} -v '/start_img/' | \
              ${GREP} -v -- '/http://' | \
              ${GREP} -v -- '\.js' | \
        ...
        # http://blog.livedoor.jp/avinfolie/archives/27770110.html
        BEGINCOUNT=`${GREP} -- '<div class="article-body-inner">' ${FILENAME} | ${WC} -l | ${TR} -d ' '`
        ENDCOUNT=`${GREP} -- '<div id="ad2">' ${FILENAME} | ${WC} -l | ${TR} -d ' '`
        if [ ${BEGINCOUNT} -gt 0 -a ${ENDCOUNT} -gt 0 ]; then
            ${CAT} ${FILENAME} | \
              ${SED} 's/></>\n</g' | \
              ${GREP} -m 1 -A 10000 -- '<div class="article-body-inner">' | \
              ${GREP} -m 1 -B 10000 -- '<div id="ad2">' | \
              ${GREP} -i -e 'jpg' -e 'jpeg' -e 'png' -e 'gif' | \
                ${GREP} -v '/start_img/' | \
              ${GREP} -v -- '/http://' | \
              ${GREP} -v -- '\.js' | \
