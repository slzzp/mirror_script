...
while [ ! -z "$1" ]; do
    ...
    while [ 1 -eq 1 ]; do
        # http://blog.livedoor.jp/samplems-bakufu/archives/51389964.html
        BEGINCOUNT=`${GREP} -- '<div class="article-body-inner">' ${FILENAME} | ${WC} -l | ${TR} -d ' '`
        ENDCOUNT=`${GREP} -- '<div class="related-articles">' ${FILENAME} | ${WC} -l | ${TR} -d ' '`
        if [ ${BEGINCOUNT} -gt 0 -a ${ENDCOUNT} -gt 0 ]; then
            ${CAT} ${FILENAME} | \
              ${SED} 's/></>\n</g' | \
              ${GREP} -m 1 -A 10000 -- '<div class="article-body-inner">' | \
              ${GREP} -m 1 -B 10000 -- '<div class="related-articles">' | \
