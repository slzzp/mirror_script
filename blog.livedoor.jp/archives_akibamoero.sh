...
while [ ! -z "$1" ]; do
    ...
    while [ 1 -eq 1 ]; do
        # http://blog.livedoor.jp/akibamoero/archives/26119623.html
        BEGINCOUNT=`${GREP} -- '<div class="article-body-inner">' ${FILENAME} | ${WC} -l | ${TR} -d ' '`
        ENDCOUNT=`${GREP} -- '<i class="article-body entry-content">' ${FILENAME} | ${WC} -l | ${TR} -d ' '`
        if [ ${BEGINCOUNT} -gt 0 -a ${ENDCOUNT} -gt 0 ]; then
            ${CAT} ${FILENAME} | \
              ${SED} 's/></>\n</g' | \
              ${GREP} -m 1 -A 10000 -- '<div class="article-body-inner">' | \
              ${GREP} -m 1 -B 10000 -- '<i class="article-body entry-content">' | \
        ...
        # http://blog.livedoor.jp/akibamoero/archives/26526615.html
        BEGINCOUNT=`${GREP} -- '<div class="article-body-inner">' ${FILENAME} | ${WC} -l | ${TR} -d ' '`
        ENDCOUNT=`${GREP} -- '<div class="article-body entry-content">' ${FILENAME} | ${WC} -l | ${TR} -d ' '`
        if [ ${BEGINCOUNT} -gt 0 -a ${ENDCOUNT} -gt 0 ]; then
            ${CAT} ${FILENAME} | \
              ${SED} 's/></>\n</g' | \
              ${GREP} -m 1 -A 10000 -- '<div class="article-body-inner">' | \
              ${GREP} -m 1 -B 10000 -- '<div class="article-body entry-content">' | \
              ${GREP} -i -e 'jpg' -e 'jpeg' -e 'png' -e 'gif' | \
              ${GREP} -v -- '/http://' | \
              ${GREP} -v -- '\.js' | \
              ${GREP} -v -- '\.html' | \
