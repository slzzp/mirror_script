...
while [ ! -z "$1" ]; do
    ...
    while [ 1 -eq 1 ]; do
        ...
        # http://blog.livedoor.jp/eroeroga/archives/6638440.html
        BEGINCOUNT=`${GREP} -- '<div class="article-body-inner">' ${FILENAME} | ${WC} -l | ${TR} -d ' '`
        ENDCOUNT=`${GREP} -- '/fbe47816.png' ${FILENAME} | ${WC} -l | ${TR} -d ' '`
        if [ ${BEGINCOUNT} -gt 0 -a ${ENDCOUNT} -gt 0 ]; then
            ${CAT} ${FILENAME} | \
              ${SED} 's/></>\n</g' | \
              ${GREP} -m 1 -A 10000 -- '<div class="article-body-inner">' | \
              ${GREP} -m 1 -B 10000 -- '/fbe47816.png' | \
              ${GREP} -i -e 'jpg' -e 'jpeg' -e 'png' -e 'gif' | \
                ${GREP} -v -- '/fbe47816.png' | \
              ${GREP} -v -- '/http://' | \
              ${GREP} -v -- '\.js' | \
