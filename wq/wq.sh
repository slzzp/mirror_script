...
while [ ! -z "$1" ]; do
    ...
    # facebook pic 9-1
    # if url: https://scontent.ftpe7-4.fna.fbcdn.net/v/t1.15752-9/494818485_2754377631618075_1021489654082731372_n.jpg?_nc_cat=105&ccb=1-7&_nc_sid=9f807c&_nc_ohc=HiuEzjtQbtgQ7kNvwEoC-LK&_nc_oc=AdmdDoZ8xOQAtTz0zYbDjk-yj-piA-O1Cn3qD1LQBjFKkxygDulCSghRnZ8zR6uCgHA&_nc_zt=23&_nc_ht=scontent.ftpe7-4.fna&oh=03_Q7cD2QHWme_U4Mr2axgX9C95aqQDhyVmjqEtNeSPF7YP3E9Oiw&oe=686665F6
    #  save file into 494818485_2754377631618075_1021489654082731372_n.jpg
    if [ '_nc_cat' = "${ARGAN}" -a 'ccb' = "${ARGBN}" -a '_nc_sid' = "${ARGCN}" -a '_nc_ohc' = "${ARGDN}" -a \
         '_nc_oc' = "${ARGEN}" -a '_nc_zt' = "${ARGFN}" -a '_nc_ht' = "${ARGGN}" -a \
         'oh' = "${ARGHN}" -a 'oe' = "${ARGIN}" ]; then
        wq_get_filename "${FILENAME}"
    ...
    # facebook pic 11-2
    # if url: https://scontent-tpe1-1.xx.fbcdn.net/v/t39.30808-6/567986965_1421537596000253_5761174368178192212_n.jpg?_nc_cat=101&ccb=1-7&_nc_sid=127cfc&_nc_aid=0&_nc_ohc=rnkJ-aZvosoQ7kNvwFpMpPB&_nc_oc=AdkWTSRC12-RdXRYSO3EtTYcJUtM050acJYe1QrEeU_GtdFL19w8I59azRNPj5MwRn1aclOq9PZ7iCJ3hU_PMJmH&_nc_zt=23&_nc_ht=scontent-tpe1-1.xx&_nc_gid=IOrN2M8jVzuGnIAPitFkAQ&oh=00_AfcI0TIBoQD2dw3gsM47Rjp_SE3Va5Tj8mF-FPyPz4Yc6w&oe=68FADB47
    #  save file into 567986965_1421537596000253_5761174368178192212_n.jpg
    if [ '_nc_cat' = "${ARGAN}" -a 'ccb' = "${ARGBN}" -a '_nc_sid' = "${ARGCN}" -a '_nc_aid' = "${ARGDN}" -a \
         '_nc_ohc' = "${ARGEN}" -a '_nc_oc' = "${ARGFN}" -a '_nc_zt' = "${ARGGN}" -a '_nc_ht' = "${ARGHN}" -a \
         '_nc_gid' = "${ARGIN}" -a 'oh' = "${ARGJN}" -a 'oe' = "${ARGKN}" ]; then
        wq_get_filename "${FILENAME}"
    ...
    # facebook pic 11-3
    # if url: https://scontent.ftpe7-4.fna.fbcdn.net/v/t39.30808-6/641624818_122235820886285088_853290561084051708_n.jpg?_nc_cat=107&ccb=1-7&_nc_sid=13d280&_nc_ohc=2cE3W95SM20Q7kNvwG6gc3W&_nc_oc=Adkbv08Un93DSLy6x4vm1R9SLXtZndBmM-75cHRhZyZA-0W7jCCNlFesIYChlWU5nKQ&_nc_zt=23&_nc_ht=scontent.ftpe7-4.fna&_nc_gid=MHTRx-ljFLmcPXNewngpog&_nc_ss=8&oh=00_AfsdA8w1BbeJ88VPXnDIYZmyrK9hBM4CR6ifufWnkvqUgQ&oe=69A84A5F
    #  save file into 641624818_122235820886285088_853290561084051708_n.jpg
    if [ '_nc_cat' = "${ARGAN}" -a 'ccb' = "${ARGBN}" -a '_nc_sid' = "${ARGCN}" -a '_nc_ohc' = "${ARGDN}" -a \
         '_nc_oc' = "${ARGEN}" -a '_nc_zt' = "${ARGFN}" -a '_nc_ht' = "${ARGGN}" -a '_nc_gid' = "${ARGHN}" -a \
         '_nc_ss' = "${ARGIN}" -a 'oh' = "${ARGJN}" -a 'oe' = "${ARGKN}" ]; then
        wq_get_filename "${FILENAME}"
